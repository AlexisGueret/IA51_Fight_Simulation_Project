...
public class MovementHandler extends Agent {
  ...
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    ...
    this.collisionHandler = ((CollisionHandler) _get);
    ...
    final AgentTask task = _$CAPACITY_USE$IO_SARL_CORE_SCHEDULES$CALLER.task("Makes agents move");
    ...
    final Procedure1<Agent> _function = (Agent it) -> {
      this.collisionHandler.Update2(0.08);
    };
    _$CAPACITY_USE$IO_SARL_CORE_SCHEDULES$CALLER_1.every(task, 5, _function);
  }
  
  private void $behaviorUnit$Restart$1(final Restart occurrence) {
  }
  ...
  @SyntheticMember
  @PerceptGuardEvaluator
  private void $guardEvaluator$Restart(final Restart occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$Restart$1(occurrence));
  }
  ...
  @SyntheticMember
  public MovementHandler(final UUID parentID, final UUID agentID) {
  ...
  @Inject
  @Deprecated
  public MovementHandler(final BuiltinCapacitiesProvider provider, final UUID parentID, final UUID agentID) {
    super(provider, parentID, agentID);
  }
  
  @SyntheticMember
  @Inject
  public MovementHandler(final UUID parentID, final UUID agentID, final DynamicSkillProvider skillProvider) {
